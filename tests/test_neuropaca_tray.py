...
def test_find_module_locates_presence_when_present() -> None:
    health = _health(state="idle")
    assert tray._find_module(health, "presence") is not None


def test_find_module_is_none_when_absent() -> None:
    health = {"ok": True, "modules": []}
    assert tray._find_module(health, "presence") is None
