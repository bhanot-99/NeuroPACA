...
- **Phase:** B1 — Core Infrastructure. **Complete** on branch `b1-core-infra` (not yet merged). B0 done. The B0 model spike is still not run (gates L4/L6/L9 fallbacks, not B1).
- **Currently working on:** B1 wrap-up. Commits `a95ce07`→`dcc60a4`. `ruff` + `mypy` (25 files) + **84 pytest** green (+3 B0-spike skips).
