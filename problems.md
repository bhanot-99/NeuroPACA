...
| T6 | **`scripts/soak_state.py`'s `rss_trend()` reports a huge, misleading leak-slope right after a daemon restart.** It fits `(last_mib - first_mib) / span` over the *longest single daemon life* segment — correct for a genuine sustained leak, but a one-time step (graph/model warm-up allocation) divided by a short elapsed window extrapolates to an absurd daily rate. Observed live 2026-09-04: RSS jumped 43→1476 MiB in a single 60 s sample right after the B9 soak's third session started, then sat flat at ~1477 MiB for 3+ hours — `rss_trend()` reported **`+5600.7 MiB/day`** in that window, surfaced verbatim in both `soak_7day.sh`'s popup and the new `scripts/soak_tray.py` widget, because both render `summarise()`'s text unchanged. Confirmed by hand against `data/soak/samples.jsonl` that RSS was not actually climbing. Same warm-up-plateau phenomenon as T2/T4, now hit a third time — a slope fit through a step function, not a sampler bug. | 🟢 mitigated 2026-09-08 (B15 soak-harness rebuild). `soak_state.warm_rss_slope()` fits the slope only over samples of the longest daemon life **after RSS crosses 500 MiB** (the BitNet GGUF has mapped in), so the one-time startup jump is outside the window. `soak_state.py assess` grades the run on the warm slope, not the raw one; the popup now prints both, labelled. The dashboard already used the same warm window (`_rss_slope_health`). The raw `rss_trend()` number is kept and shown for context — a genuine leak that starts *after* warm-up still moves it. |
