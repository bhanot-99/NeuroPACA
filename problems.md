...
**L9 `$?` — RESOLVED (D-12, B5, 2026-09-01).** Dual-model routing: a Qwen2.5-3B-Instruct Q4 GGUF serves the interactive `$` / `$?` path (`BitNetRuntime` gained an optional second backend, one `_inference_lock`); 2B4T stays on the always-on loop. `$?` still runs behind a per-call GBNF grammar (`{insight, cited_nodes, confidence}`) and a hard `parse_answer` grounding gate — an ungrounded sentence is discarded and L9 answers from an extractive template instead. Real-model quality eval is carried to the target box.
