...
| T6 | **`scripts/b9_soak_state.py`'s `rss_trend()` reports a huge, misleading leak-slope right after a daemon restart.** It fits `(last_mib - first_mib) / span` over the *longest single daemon life* segment — correct for a genuine sustained leak, but a one-time step (graph/model warm-up allocation) divided by a short elapsed window extrapolates to an absurd daily rate. Observed live 2026-09-04: RSS jumped 43→1476 MiB in a single 60 s sample right after the B9 soak's third session started, then sat flat at ~1477 MiB for 3+ hours — `rss_trend()` reported **`+5600.7 MiB/day`** in that window, surfaced verbatim in both `b9_soak_7day.sh`'s popup and the new `scripts/b9_soak_tray.py` widget, because both render `summarise()`'s text unchanged. Confirmed by hand against `data/b9_soak/samples.jsonl` that RSS was not actually climbing. Same warm-up-plateau phenomenon as T2/T4, now hit a third time — a slope fit through a step function, not a sampler bug. | 🟡 open — surfaces live in the popup and tray; no code changed yet. Fix candidates: skip the first N minutes of a session before the trend window starts; fit the slope only over the flat tail (e.g. drop the leading segment until the point-to-point delta stays under some MiB/sample threshold); or report a "since-warm-up" slope alongside the current one so a real leak starting right after a restart isn't masked by whichever window wins. Whichever lands should be checked against the T2/T4 shape. |
