...
| T4 | **The scheduler stalls the event loop ~320–370 ms per tick with a 10k-node graph.** Found by `scripts/measure_loop_lag.py --big-graph` (B4-prep stress suite, 2026-08-31): default run (small graph) max loop lag **2 ms** ✅, but with the 10k fixture + `graph_save_interval_seconds=2` the probe sees a **320–370 ms** spike every tick. Cause (same as T2): `GraphMemory.save()` builds a ~3 MB `json.dumps(indent=2, sort_keys=True)` string **on the loop under `_lock`** (`graph_memory.py` ~line 209), and `recalculate_importance()` iterates all 10k nodes **on the loop under `_lock`**. `tests/stress/test_loop_lag.py` guards only the small-graph baseline (the real B4 gate). | 🔴 open — **must fix before B4** (in-process `infer_async` on a loop that already stalls 370 ms would freeze the daemon). Fix menu: (a) move `json.dumps` into the `to_thread` worker (serialise dict under the lock, stringify off-loop); (b) stream/`orjson` the write; (c) chunk `recalculate_importance()` or move it to `to_thread` over a graph snapshot; (d) raise the default save interval. A real graph is far smaller than 10k early on, so this is not yet a live-daemon problem — but B4 makes the loop budget unforgiving. Details: `memory.md`, run `uv run python scripts/measure_loop_lag.py --big-graph`. |
