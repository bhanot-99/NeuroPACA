...
- **Dual-trust membrane:** Video titles pass strictly through configurable regex allowlists (`data/media_title_patterns.default.toml`); unmatched browser titles are dropped completely with zero raw title leakage. Genuine music players with populated `xesam:artist` or `xesam:album` are trusted directly.
- **Scope boundary:** Watching and listening only; reading explicitly excluded.
...
- **Episodic & Graph integration:** Graph schema v11 with `NodeType.SERIES` and `domain:media` hub (12 hubs total); `EpisodeKind.MEDIA_SPAN` for active playback sessions and `EpisodeKind.MEDIA_POSITION_FACT` superseded on identity change (churn-suppressed) or span close (pause/stop/track-change/shutdown, force-refreshing the real stopping position rather than freezing it at first detection — itself gated so a close observing nothing new never duplicates a fact).
...
- **Cross-cutting fix found while hardening this phase:** `EpisodeStore.flush()` (S0 infrastructure, not S3-specific) could return success before a write had actually landed — it fast-pathed on `Queue.empty()`, which the writer task makes true the instant it dequeues a job, before the blocking insert (running in a thread) finishes. Every phase's `flush()`-then-read test pattern was exposed to this; fixed to always wait on `Queue.join()`, with a new deterministic regression test in `tests/test_episodes.py`. Full account in the dossier §21.25.
