...
| ✅ | a review period in dry-run with zero false positives before any tier goes live. Agreed as a 24 h dogfooding soak under `neuropaca.soak.toml`, but **all three soak attempts logged zero proposals** — attempt 1 (2026-09-01 16:48→20:20 IST) aborted for a reboot; attempt 2 (2026-09-01 22:02→2026-09-02 20:35 IST); attempt 3 (2026-09-03 07:45 IST, clean window). Root cause, now accepted as structural: the only pressure path a headless `systemd --user` daemon reaches is `HighLoadPattern` (the Wayland activity collector self-disables — `no $WAYLAND_DISPLAY`), and a normal day does not pin CPU > 90 % for 5 min while touching a watched path. An empty soak log can't tell a correct-quiet pipeline from a broken one. **Met instead by the positive control** (`scripts/b7_positive_control.py`, thresholds byte-identical to the soak): a bounded synthetic HighLoad load through a throwaway daemon. Run 2026-09-02 20:02→00:49 UTC (~4.5 h): **60 attempts, 60/60 attempt+result pairs, 0 executed effects, 0 high-tier proposals**, every safe-tier proposal tracing to `L3 high_load: cpu 100% for 5 samples`. Zero high-tier ⇒ nothing to judge ⇒ the zero-false-positive rule holds mechanically. Evidence: `spikes/b7_positive_control/`. Validated by `scripts/finalize_b7.sh` (rewritten — validates, does not merge; B7 finalised by PR). |
...
| If… | Then… |
| --- | --- |
| ✅ | `spawn_node()` never takes the graph past `max_ephemeral_nodes`, checked **before** the mutation. Structural, not tuned: the count-then-create pair is inside the module's own lock. **Target box: 60 spawns against a cap of 12 → 12 granted / 12 live (0.5 ms), and 60 *simultaneous* spawns → 12 granted.** Unit: `test_spawn_node_never_exceeds_the_ephemeral_cap`, `test_the_cap_holds_under_a_concurrent_burst`. |
| ✅ | apoptosis reaps every ephemeral node older than `agent_idle_ttl_days` and **nothing else**. **Target box, at the production 14 d constant: 6 aged / 6 fresh → 6 reaped, 6 survive, edges 12→6, 0 dangling, `app:webpack` untouched, node and edge counts exact.** `kill_node` refuses a non-ephemeral node. The marker survives a save/reload (6/6) — without which a restart would make every ephemeral node look permanent and the sweep would silently stall. Unit: `test_apoptosis_reaps_past_the_ttl_and_spares_everything_else`, `test_apoptosis_leaves_no_dangling_edges`, `test_the_ephemeral_marker_survives_a_save_and_reload`, `test_apoptosis_runs_on_boot`. |
| ✅ | an `ACTION_PROPOSAL` published by L8 is instantiated and gated by L7's single `SafetyGate`. **Target box: gate executed 1, node written, `ACTION_PROPOSAL_RESULT` ok; an unknown `action_type` refused (`not proposable`) without reaching the gate; 4 audit lines = attempt+result for both.** A dangerous proposal still faces the tier gate and the confirmation. Unit: `test_an_action_proposal_routes_through_l7s_single_gate`, `test_an_unknown_action_type_is_refused_and_audited`, `test_a_malformed_proposal_is_refused_not_raised`, `test_a_dangerous_proposal_still_obeys_the_tier_gate`. |
| ✅ | an agent that overruns `agent_wall_clock_budget_seconds` is cancelled cleanly. **Target box: a 5 s body under a 1 s budget abandoned at 1.01 s, `AGENT_COMPLETED` outcome `'timeout'`, module health still ok, graph consistent.** Unit: `test_an_agent_over_its_wall_clock_budget_is_cancelled_cleanly`, `test_stop_cancels_a_running_agent_and_closes_its_record`. |
| ✅ | a spawn requested while `max_concurrent_agents` are already running is **refused and logged**, never queued. **Target box: 60 crossings against a cap of 1 → 1 spawned, 59 refused, 1 in flight, and no second agent after the first released.** Unit: `test_a_spawn_over_the_concurrency_cap_is_refused_never_queued`. |
...
| 4 | The **7-day soak** completes, having first passed the 1-hour live gate, run under `systemd-inhibit --what=sleep:idle`. Subsumes the carried B1 T2 / B2 T3 / B4 windows. | `scripts/b9_soak_gate.sh` (gate) then `neuropaca-b9-soak.service` → `scripts/b9_soak_7day.sh` — **gate PASSED 2026-09-03 18:52 UTC; soak started 18:53 UTC** |
...
Criteria 1–3 and 5–7 are met by the test suite. **Criterion 4 is the only one that cannot be met from a keyboard** — it needs a week on the target box, gated by `scripts/b9_soak_gate.sh`.

**Three things had to be fixed on 2026-09-03 before the unit could even start**, none of which the test suite could have caught, because all three are properties of this machine rather than of the code:

1. **`neuropaca.toml` did not exist.** The unit's `Environment=NEUROPACA_CONFIG=__REPO__/neuropaca.toml` pointed at a file nothing had ever created — `neuropaca.soak.toml` said it "will ship" and it never did. `neuropaca doctor` reported `config INVALID` and the daemon could not have started at all. It now ships with `action_dry_run = true` and `action_enabled_tiers = ["safe"]`; the `dangerous` tier stays off until a completed soak has earned it (D-14(c)).
2. **The unit had never been installed**, so the `graphical-session.target` binding that supersedes the B7 Wayland misdiagnosis was verified only in the reasoning. Installed and started: the daemon process has `WAYLAND_DISPLAY`, the L9 socket binds under `ProtectSystem=strict` (BL-1, live-confirmed), and `activity ✓ idle✓ window✓` — the collector that was silent through three B7 soaks is alive.
3. **`journalctl --user` returns "No journal files were found."** journald ships `Storage=auto` and `/var/log/journal` does not exist, so the journal is volatile with no per-user files. The gate's collector and activity checks grepped exactly that, which means they would have read zero activity from an empty journal and refused the soak — or passed a soak that recorded a week of zeros for a healthy system. Both checks, and the whole sampler, now read `neuropaca health` over the L9 socket instead: structured, authoritative, and free of that dependency. *(Making the journal persistent needs root and is not required; the socket path is the better measurement regardless.)*

**Running it (BL-5, revised).** The soak is no longer one uninterrupted invocation. `neuropaca-b9-soak.service` starts it with `graphical-session.target` and stops it with the session, so it comes up at boot and shuts down cleanly before the machine does; the driver traps `SIGTERM` and closes the session in a ledger. A week therefore arrives as a *sequence of sessions across power cycles*, and `scripts/b9_soak_state.py` adds them up.

> **Completion is 7 days of accrued runtime, not 7 days of calendar.** A box powered off overnight ages no process, and counting those hours would let a 3.5-day soak claim a 7-day result — which is exactly how the B2 soak reached 11 h of a 24 h window. `systemd-inhibit --what=sleep:idle` still wraps the driver for the same reason. An unclean shutdown leaves a session open; the next boot heals it from the last heartbeat and labels it `unclean`, rounding runtime **down** rather than crediting hours the machine spent switched off.
...
**Gate result, 2026-09-03 (60 min window).** Passed. What moved was not an app switch but the idle path itself: `activity_edges` went 0 -> 2 (an idle edge and an active edge) from `ext-idle-notify-v1`, unprompted -- the transition B7 spent three soaks failing to observe. RSS moved with it, 42.7 MiB -> 1476.1 MiB, because going idle woke L6's DMN and lazily loaded BitNet. That figure is worth keeping: B4's `soak_test_b4.py` recorded **1477 MiB flat** for the same model, so two independent measurements a phase apart agree to within a megabyte.

> **A known narrowness in check 5, recorded rather than papered over.** It requires an idle transition or an app switch, and `_on_window_switch` returns early unless `app_id` actually changes (`sensing/activity/collector.py:139`). Someone working continuously inside a single application for the whole window therefore produces neither, and the gate would fail a demonstrably healthy system. This run passed on an idle edge with roughly seven minutes to spare, which is closer than it should be. If the gate is ever re-run, widen it to accept the L2 snapshot buffer or the graph advancing as well -- both prove L2/L3 are alive without depending on how the desk happens to be used that hour.
