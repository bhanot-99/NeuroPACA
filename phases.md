...
| ⏳ | a review period in dry-run with zero false positives before any tier goes live. `scripts/validate_b7_dryrun.py` measures it: it proves the log is complete and effect-free and lists every autonomous proposal with its reason for human judgement. Needs an agreed duration and a signed-off run on the target box. |
