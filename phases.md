...
**Exit:** ✅ backend loads lazily / self-disables cleanly · ✅ `measure_loop_lag.py --infer-stress` — no stall > 50 ms across a ~10 s inference · ✅ every stored insight `traces_to_evidence()` (≥ 1 snapshot + ≥ 1 node) · ✅ `test_gating_drops_at_least_half_under_a_storm`. ⏳ real-model run of `--infer-stress` + a short soak once `.[llama]` is installed on the target box.
