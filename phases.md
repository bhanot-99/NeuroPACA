...
| 🟡 | a review period in dry-run with zero false positives before any tier goes live. **Agreed as a 24 h dogfooding soak** under `neuropaca.soak.toml` (`action_dry_run = true` with **both** tiers enabled — everything proposed, nothing possible; real models, Wayland activity on, `watch_paths = [~/NeuroPaca]`). Acceptance: **zero false positives among high-tier proposals**, every safe-tier proposal traceable to its diagnosis spike, audit complete and effect-free. Analysed by `scripts/validate_b7_dryrun.py --require-hours 24`, which gates on window length and splits high- from safe-tier proposals for judgement. **Two attempts so far, both inconclusive:** attempt 1 (2026-09-01 16:48→20:20 IST, 3h32m) aborted for a reboot; attempt 2 (2026-09-01 22:02→2026-09-02 20:35 IST, 22h33m) stopped by user decision short of the 24h mark. **Both produced zero action proposals — `data/actions.jsonl` was never created either time.** Root cause: `data/app_map.default.toml` has no browser entries, so no browser-focused work can ever satisfy `FocusSessionPattern`, and `IdlePattern`/`DistractionPattern` signals structurally carry no `related_node_ids` and can't reach L4/L5 regardless. Fix identified (add the browser's `app_id` to the map) but not yet applied; criterion 5 remains open pending a third attempt. |
