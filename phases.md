...
| 4 | The **7-day soak** completes, having first passed the 1-hour live gate, run under `systemd-inhibit --what=sleep:idle`. Subsumes the carried B1 T2 / B2 T3 / B4 windows. | `scripts/b9_soak_gate.sh` (gate) then `neuropaca-b9-soak.service` → `scripts/b9_soak_7day.sh` — **gate run 2026-09-03, soak pending** |
