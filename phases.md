...
Criteria 1–3 and 5–7 are met by the test suite. **Criterion 4 is the only one that cannot be met from a keyboard** — it needs a week on the target box, gated by `scripts/soak_gate.sh`.
...
2. **The unit had never been installed**, so the `graphical-session.target` binding that supersedes the B7 Wayland misdiagnosis was verified only in the reasoning. Installed and started: the daemon process has `WAYLAND_DISPLAY`, the L9 socket binds under `ProtectSystem=strict` (BL-1, live-confirmed), and `activity ✓ idle✓ window✓`. *(That `window✓` was still lying at the time — B15 §2c: `health()` printed it for a source that had gone deaf. Fixed in B15: `window✓` now means the poll-pump is live, and a source that started then died drags the module unhealthy.)*
...
**Running it (BL-5, revised).** The soak is no longer one uninterrupted invocation. `neuropaca-soak.service` starts it with `graphical-session.target` and stops it with the session, so it comes up at boot and shuts down cleanly before the machine does; the driver traps `SIGTERM` and closes the session in a ledger. A week therefore arrives as a *sequence of sessions across power cycles*, and `scripts/soak_state.py` adds them up.
